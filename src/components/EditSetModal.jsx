  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
        {presets.map((p) => (
          <button key={p.value} type="button" title={p.label} onClick={() => onChange(p.value)} style={{ width: 32, height: 32, borderRadius: '50%', background: p.value, border: value.toLowerCase() === p.value.toLowerCase() ? '3px solid var(--rose-dark)' : '2px solid rgba(0,0,0,0.12)', cursor: 'pointer', transition: 'transform 0.12s, border 0.12s', transform: value.toLowerCase() === p.value.toLowerCase() ? 'scale(1.18)' : 'scale(1)', boxShadow: '0 1px 4px rgba(0,0,0,0.1)', flexShrink: 0 }} />
        ))}
      </div>
      <div style={{ display: 'flex', gap: 8, alignItems: 'stretch' }}>
        <div onClick={() => pickerRef.current?.click()} title="Clique para abrir o seletor de cor" style={{ flex: 1, height: 42, borderRadius: 'var(--radius-sm)', background: value, border: '1.5px solid var(--card-border)', cursor: 'pointer', position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <span style={{ fontSize: '0.7rem', fontWeight: 800, fontFamily: 'monospace', color: isLight(value) ? 'rgba(0,0,0,0.45)' : 'rgba(255,255,255,0.6)', letterSpacing: '0.04em', pointerEvents: 'none', userSelect: 'none' }}>🎨 abrir seletor</span>
          ...
          {filtered.length === 0 ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
              {filtered.map((opt) => {
                ...
                return (
                  <button key={opt.value} type="button" onClick={() => handleSelect(opt)} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '11px 14px', borderRadius: 'var(--radius-sm)', border: isSelected ? '1.5px solid var(--rose)' : '1.5px solid transparent', background: isSelected ? 'var(--rose-light)' : 'rgba(255,255,255,0.5)', cursor: 'pointer', textAlign: 'left', transition: 'background 0.1s, border-color 0.1s', gap: 12 }}
...
/* ─── ImageUploadField ─── */
function ImageUploadField({ imageFile, imagePreview, existingUrl, onFileChange, onClear }) {
  const inputRef = useRef();
  const displayPreview = imagePreview || existingUrl;
      ...
      {displayPreview ? (
        <div style={{ position: 'relative' }}>
          <div style={{ borderRadius: 'var(--radius-sm)', overflow: 'hidden', aspectRatio: '16/9', background: 'var(--gray-light)' }}>
            <img src={displayPreview} alt="preview" style={{ width: '100%', height: '100%', objectFit: 'cover' }} onError={(e) => { e.target.style.display = 'none'; }} />
          </div>
        ...
        <div onClick={() => inputRef.current?.click()} style={{ border: '2px dashed var(--card-border)', borderRadius: 'var(--radius-sm)', padding: '24px 16px', textAlign: 'center', cursor: 'pointer', transition: 'border-color 0.15s, background 0.15s', background: 'rgba(255,255,255,0.5)' }}
        ...
        >
          ...
          <div style={{ fontSize: '0.73rem', color: 'var(--gray)', marginTop: 3 }}>JPG, JPEG, PNG ou WEBP</div>
        </div>
      )}
...
export default function EditSetModal({ set, onClose, onSaved }) {
  ...
  // Imagem — pode ser uma nova do dispositivo ou manter a URL existente
  ...
  const handleFileChange = (file) => {
    ...
    setExistingUrl(''); // descarta URL antiga ao selecionar nova
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // 2. Se tiver nova imagem selecionada, faz upload
      if (imageFile) {
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div style={{ borderRadius: 'var(--radius)', overflow: 'hidden', border: '1.5px solid var(--card-border)', marginBottom: 4 }}>
            <div style={{ background: bgColor, padding: '16px 18px', transition: 'background 0.2s' }}>
              {(imagePreview || existingUrl) && (
                <div style={{ width: '100%', aspectRatio: '16/9', borderRadius: 8, overflow: 'hidden', marginBottom: 10 }}>
                  <img src={imagePreview || existingUrl} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} onError={(e) => { e.target.style.display = 'none'; }} />
                </div>
              )}
